...
from cs336_basics.tokenizer import RegexTokenizer
...
def get_tokenizer(
    ...
    tokenizer._load_from_raw_data(vocab, merges)
    if special_tokens:
        ...
        for token in special_tokens:
            ...
            if token_bytes in bytes_to_id:
                special_dist[token] = bytes_to_id[token_bytes]
            else:
                new_id = len(vocab) + len(special_dist)
                special_dist[token] = new_id
        tokenizer.register_special_tokens(special_dist)
    return tokenizer
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    returned_vocab = tokenizer.vocab.copy()
    base_id = len(returned_vocab)
    for i, token in enumerate(special_tokens):
        returned_vocab[base_id + i] = token.encode("utf-8")

    returned_merges = []
    for (id0, id1) in tokenizer.merges.keys():
        bytes0 = tokenizer.vocab[id0]
        bytes1 = tokenizer.vocab[id1]
        returned_merges.append((bytes0, bytes1))
    return returned_vocab, returned_merges
